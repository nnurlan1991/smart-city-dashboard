# SmartCity-KZ Dashboard MVP — план сборки для агентной IDE

## Контекст

Переделываем существующий мокап (`system-oicg/`) под Методику v2.4.
Визуальный стиль сохраняется: тёмная тема, Inter, Chart.js, Lucide.
Данные полностью переписываются — все числа соответствуют Методике.
...
```
smartcity-kz/
├── index.html          # Разметка всех страниц и модалей
├── style.css           # CSS (тёмная + светлая тема, адаптив)
├── data.js             # Все данные (инициативы, города, регионы, руководители)
├── app.js              # Логика навигации, рендеры, события
├── charts.js           # Chart.js конфигурации
├── maps/
│   └── map.svg         # Карта Казахстана (без изменений из оригинала)
└── avatars/
    ├── placeholder.svg  # SVG-аватар-заглушка (генерируется)
    └── README.md
```

---

## Шаг 1 — `data.js`: полные данные по Методике

### 1.1 Инициативы (20 базовых + 3 дополнительные)
...
## Шаг 2 — `avatars/placeholder.svg`

Генерируй SVG-аватар программно по инициалам. Функция в `app.js`:
...
## Шаг 3 — `index.html`: структура страниц

Сохраняй существующую разметку header, nav, модалей.
Измени только:

1. `<title>` → `Дашборд SmartCity-KZ | Методика v2.4`
2. Добавь в `<head>`:
   ```html
   <script src="data.js"></script>
   ```
3. Навигация — 6 вкладок:
   - `data-page="overview"` — Обзор (главный экран)
   - `data-page="ratings"` — Рейтинги (Р/О/РЦ)
   - `data-page="initiatives"` — Инициативы
   - `data-page="cities"` — Города
   - `data-page="leaders"` — Руководители
   - `data-page="analytics"` — Аналитика

4. Добавь новый `<div id="page-ratings" class="page-section" style="display:none">` с тремя переключателями внутри.

5. Добавь `<div id="page-analytics" class="page-section" style="display:none">`.

---

## Шаг 4 — `app.js`: полная перезапись логики

### 4.1 Главный экран (overview)
...
### 4.2 Страница рейтингов (ratings)

```javascript
function renderRatings() {
  // Три кнопки-таба: Р-класс | О-класс | RegionScore
  // При клике — рендерит соответствующую таблицу

  // Р-класс: 3 строки (Алматы, Астана, Шымкент)
  //   Колонки: Место | Город | TotalScore | AI Score | Управление | Безопасность | Транспорт | Экология | ЖКХ | Тренд
  //   TotalScore = cityScore100 + bonusPoints - penalty

  // О-класс: RegionScore = 0.6 * Score_oblast + 0.4 * Score_RC
  //   Для мокапа Score_oblast = cityScore100 областного центра
  //   Показываем обе подоценки

  // Клик на строку → открывает карточку города (существующий city-detail-modal)
}
```

### 4.3 Страница инициатив
...
## Шаг 5 — `charts.js`: правильные данные для Chart.js
...
## Шаг 6 — `style.css`: минимальные правки

Сохранить весь оригинальный CSS. Добавить только:

```css
/* Новые элементы */
.rag-green  { color: #10b981; }
.rag-yellow { color: #f59e0b; }
...
.func-check { color: #10b981; }
.func-cross { color: #6b7280; }
.func-na    { color: #6b7280; font-style: italic; }

.ai-level-badge {
  display: inline-flex; align-items: center; gap: 4px;
  padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600;
}
.ai-0 { background: rgba(107,114,128,0.2); color: #9ca3af; }
.ai-1 { background: rgba(59,130,246,0.2);  color: #60a5fa; }
.ai-2 { background: rgba(234,179,8,0.2);   color: #facc15; }
.ai-3 { background: rgba(16,185,129,0.2);  color: #34d399; }

.spec-tag {
  font-size: 11px; padding: 2px 8px; border-radius: 4px;
  background: rgba(31,111,235,0.15); color: #79c0ff;
...
/* Overrides для leader-card */
.leader-avatar {
  width: 72px; height: 72px; border-radius: 50%;
  object-fit: cover; border: 2px solid var(--border-color);
}

.kpi-rag-table th { font-size: 11px; }
.kpi-rag-table td { font-size: 13px; vertical-align: middle; }
```

---

## Шаг 7 — Чек-лист перед демо

- [ ] `data.js` загружается первым в `index.html`
- [ ] Все `Math.random()` удалены из `script.js`
- [ ] Локальные file:// пути к аватарам заменены на `makeAvatar(initials)`
- [ ] TotalScore везде считается: `city.cityScore100 + city.bonusPoints - city.penalty`
- [ ] Формула AI Score: `0.35*P1 + 0.30*P2 + 0.20*P3 + 0.15*DataLake`
- [ ] Матрица городов использует `initiativeStatus[id].kpiRag` для цвета
- [ ] Модал инициативы показывает KPI из `initiative.kpi[]` (target_R/O/RC)
- [ ] Модал инициативы показывает функционал из `initiative.functional[]`
- [ ] Карточка города показывает реального куратора и руководителя УЦ
- [ ] Рейтинг руководителей — только `initials`-аватары, без file:// путей
- [ ] Применимость NA (серая ячейка) отображается для РЦ где R:false, O:false

---

## Примечание для агента

Этот файл — план, не финальный код. Агент должен:
1. Создать файл `data.js` с данными выше (полностью)
2. Переписать `script.js` → `app.js` используя функции из плана
3. Сохранить `style.css` + добавить CSS из Шага 6
4. Обновить `index.html` по Шагу 3
5. Создать новый `charts.js`
6. Проверить открытие в браузере без ошибок в консоли
